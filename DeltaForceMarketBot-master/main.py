...
# 时间窗口相关配置
ENABLE_ACTIVE_WINDOW = SYS_CONFIG.get("enable_active_window", False)
ACTIVE_WINDOW = SYS_CONFIG.get("active_window", {"start": "00:00", "end": "23:59"})
INACTIVE_SLEEP_SECONDS = SYS_CONFIG.get("inactive_sleep_seconds", 30)

def parse_hhmm_to_minutes(hhmm: str) -> int:
    """将 HH:MM 转换为自午夜以来的分钟数"""
    try:
...
def is_within_active_window(now: datetime | None = None) -> bool:
    """判断当前时间是否在激活窗口内，支持跨午夜区间

    例如 start=23:00, end=02:00 视为跨日，23:00-24:00 与 00:00-02:00 都为有效
    """
    if not ENABLE_ACTIVE_WINDOW:
        return True

    if now is None:
        now = datetime.now()

    start_minutes = parse_hhmm_to_minutes(str(ACTIVE_WINDOW.get("start", "00:00")))
    end_minutes = parse_hhmm_to_minutes(str(ACTIVE_WINDOW.get("end", "23:59")))
    ...
    if start_minutes <= end_minutes:
        ...
        # 跨午夜区间，例如 23:00-02:00
        return current_minutes >= start_minutes or current_minutes <= end_minutes

def is_now_in_window(window_conf: dict | None) -> bool:
    """判断当前时间是否在给定的窗口内；若未配置窗口，默认允许

    window_conf 形如 {"start": "HH:MM", "end": "HH:MM"}
    支持跨午夜区间
    """
    if not window_conf:
        return True
    try:
        start = str(window_conf.get("start", "00:00"))
        end = str(window_conf.get("end", "23:59"))
...
def process_category(
    ...
    # 若该类别下无任一物品处于允许的时间窗口，则无需展开菜单，直接跳过
    has_item_in_window = any(
        is_now_in_window(item.get("active_window"))
        for items in item_info.values()
        for item in items
    )
    if not has_item_in_window:
        logger.info(f"{category_name} 当前无物品处于允许的时间段内，跳过该类别")
        return True
    ...
    for type_name, items in item_info.items():
        ...
        for item in items:
            ...
            # 若物品配置了时间窗口且当前不在窗口内，则跳过
            if not is_now_in_window(item.get("active_window")):
                logger.info(f"{item['name']} 当前不在物品允许的时间段内，跳过")
                continue
...
def main():
    ...
    while True:
        if IS_RUNNING and not IS_PAUSED:
            # 若启用时间窗口，非活动时间内则休眠并跳过一轮
            if not is_within_active_window():
                logger.info("当前不在活动时间窗口内，等待...")
